trigger OppLineSyncTrigger on OpportunityLineItem (before insert, after insert, after update) {
    ...
    String oliQuery = 'select Id, OpportunityId, PricebookEntryId, Quantity, ServiceDate, SortOrder' + oliFields + ' from OpportunityLineItem where Id in (' + oliIds + ') order by OpportunityId, SortOrder ASC';
    ...
    if (quoteIds != '') {
        ...
        String qliQuery = 'select Id, QuoteId, PricebookEntryId, Quantity, ServiceDate, SortOrder' + qliFields + ' from QuoteLineItem where QuoteId in (' + quoteIds + ') order by QuoteId, SortOrder ASC';   
