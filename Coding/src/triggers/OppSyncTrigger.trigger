trigger OppSyncTrigger on Opportunity (after update) {
    ...
    if (oppIds != '') {
        ...
        if (quoteIds != '') { 
            ...
            System.debug('In OppSyncTrigger- quoteQuery = '+quoteQuery);
            
            List<Quote> quotes = Database.query(quoteQuery);
            System.debug('In OppSyncTrigger- quotes List = '+quotes);
    ...
    if (!startSyncQuoteMap.isEmpty()) {
        ...
        String qliQuery = 'select Id, QuoteId, PricebookEntryId, Quantity, SortOrder' + qliFields + ' from QuoteLineItem where QuoteId in (' + syncQuoteIds + ') order by QuoteId, SortOrder ASC';           
        String oliQuery = 'select Id, OpportunityId, PricebookEntryId, Quantity, SortOrder' + oliFields + ' from OpportunityLineItem where OpportunityId in (' + syncOppIds + ') order by OpportunityId, SortOrder ASC';   
